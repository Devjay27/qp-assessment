  ...
  try {
    ...
    if (!items || !Array.isArray(items) || items.length === 0 || !userName) {
      return response
        ...
        .json({
          success: false,
          message: "Invalid or empty items array or missing user name",
        });
    }
    ...
    for (const item of items) {
      ...
      const fetchGroceryNameQuery =
        "SELECT name AS groceryName FROM grocery_items WHERE id = ?";
          ...
          db.query(
            ...
            (updateErr, updateResult) => {
              ...
              const orderHistoryQuery =
                "INSERT INTO order_history (user_name, grocery_name, quantity) VALUES (?, ?, ?)";
              db.query(
                orderHistoryQuery,
                [userName, groceryName, quantity],
                (orderHistoryErr, orderHistoryResult) => {
                  if (orderHistoryErr) {
